...
export const LogoTrackSection: React.FC<LogoTrackBlock> = (p) => {
  ...
  return (
    ...
    >
      ...
      <div
        className="relative flex gap-[40px] md:gap-[80px] overflow-x-auto w-full no-scrollbar max-h-min h-[50px] md:h-[80px]"
        ref={scrollRef}
      >
        {logos &&
          logos.map((l, i) => {
            return (
              <div
                key={`logo-${i}`}
                className="min-w-[140px] md:min-w-[300px] flex justify-center items-end"
              >
                <img
